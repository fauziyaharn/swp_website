...
class LocalIntentPipeline:
    ...
    def predict(self, text: str, max_len: int = 64) -> Dict:
        ...
        # Conservative rule-based fallback for "tanya_kemungkinan".
        # Only override when model confidence is low AND the text looks like a question + contains budget/guest info.
        lower = text.lower()
        question_indicators = ['cukup', 'bisa', 'apakah', 'berapa', 'mampu', 'apa', 'gimana', 'kira', 'kira-kira', '?']
        contains_question = any(ind in lower for ind in question_indicators) or text.strip().endswith('?')
        has_budget_or_guests = bool(slots.get('budget_min') or slots.get('budget_max') or slots.get('jumlah_tamu'))

        overridden = False
        override_reason = None
        model_pred = INTENT_LIST[pred_id]
        model_confidence = float(probs[pred_id])
        intent_pred = model_pred
        probs_dict = {label: float(probs[i]) for i, label in enumerate(INTENT_LIST)}

        # threshold: only override when model is not confident
        # AND the question contains explicit question cues (prevent aggressive overrides)
        CONFIDENCE_THRESHOLD = 0.6
        explicit_indicators = ['kira', 'kira-kira', 'kira kira', 'cukup ga', 'cukup', 'apa', 'apakah']
        contains_explicit_question = any(ind in lower for ind in explicit_indicators) or text.strip().endswith('?')

        if (contains_explicit_question and contains_question and has_budget_or_guests and model_confidence < CONFIDENCE_THRESHOLD
            and model_pred != 'tanya_kemungkinan'):
            intent_pred = 'tanya_kemungkinan'
            probs_dict = {lbl: 0.0 for lbl in INTENT_LIST}
            probs_dict['tanya_kemungkinan'] = 1.0
            overridden = True
            override_reason = f'rule:low_confidence_question({model_confidence:.2f})'
